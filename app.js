(function() {
    ...
    document.getElementById('btn-bkn-login').onclick = function(e) {
        ...
        if(!window.location.hostname.includes('bkn.go.id')) {
            alert("Gagal: Fitur ini hanya berfungsi jika Anda sudah berada di website BKN (bkn.go.id).");
            return;
        }
        ...
        for(let span of spans){ 
            if(span.textContent.trim().toLowerCase() === 'login'){ 
                const p = span.closest('div.text-center.flex.flex-col'); 
                if(p){ p.click(); found = true; break; } 
            } 
        }
        if(!found) {
            const btnMasuk = document.getElementById('btn-login');
            if(btnMasuk) { btnMasuk.click(); found = true; }
        }
        
        if(found) {
            this.innerHTML = '✅ Sistem Aktif!';
            this.style.backgroundColor = '#4CAF50';
            this.style.borderColor = '#4CAF50';
            this.style.color = '#ffffff';
        } else {
            alert("Gagal: Tombol Login awal tidak ditemukan di halaman ini. Pastikan Anda berada di halaman depan BKN.");
        }
    };
    ...
    document.getElementById('btn-bkn-pegawai').onclick = async function(e) {
        ...
        if(!window.location.hostname.includes('bkn.go.id') || !usernameInput) {
            alert("Gagal: Halaman Login SSO BKN (kolom username) tidak ditemukan!");
            return;
        }
        if(document.getElementById('dropdown-siasn')) {
            alert("Pilihan pegawai sudah dimunculkan.");
            return;
        }
        ...
        try {
            ...
            if(!res.ok) throw new Error("Gagal membaca Data Pegawai"); 
            ...
            for(let i=0; i<lines.length; i++){
                ...
                if(row.length>=2){
                    dataPegawai.push({
                        nama: row[0]?row[0].trim().replace(/^"|"$/g,''):'',
                        nip: row[1]?row[1].trim().replace(/^"|"$/g,''):'',
                        password: row[2]?row[2].trim().replace(/^"|"$/g,''):''
                    });
                }
            }
            
            const selectContainer = document.createElement('div'); 
            selectContainer.id = 'dropdown-siasn';
            ...
            selectElement.style.cssText = 'width:100%;padding:8px;border:1px solid #ccc;border-radius:4px;background-color:#f0f8ff;font-size:14px;';
            selectElement.innerHTML = '<option value="">-- Silakan Pilih Nama --</option>';
            dataPegawai.forEach(p=>{ 
                const opt = document.createElement('option'); 
                opt.value = p.nip||p.nama; 
                opt.text = !p.nip ? p.nama+" (NIP Kosong)" : p.nama; 
            ...
            selectElement.addEventListener('change',function(ev){
                ...
                const p = dataPegawai.find(x=>x.nip===val||x.nama===val);
                if(p && p.nip){
                    usernameInput.value=p.nip; 
                    usernameInput.dispatchEvent(new Event('input',{bubbles:true})); 
                    usernameInput.dispatchEvent(new Event('change',{bubbles:true}));
                    if(passwordInput && p.password){ 
    ...
    document.getElementById('btn-bkn-otp').onclick = async function(e) {
        ...
        const otpInput = document.getElementById('otp') || document.getElementById('totp') || document.querySelector('input[autocomplete="one-time-code"]');
        
        if(!window.location.hostname.includes('bkn.go.id') || (!nipDisplay && !otpInput)) {
            alert("Gagal: Halaman Input OTP BKN tidak ditemukan!");
            return;
        }
        
        const btn = this;
        const originalText = btn.innerHTML;
        btn.innerHTML = '⏳ Membuat OTP...';
        
        try {
            const nip = nipDisplay ? nipDisplay.innerText.trim() : prompt("Sistem tidak mendeteksi NIP di layar. Masukkan NIP Anda:");
            if(!nip) throw new Error("NIP diperlukan untuk mencari Secret Key.");
            ...
            for(let i=0; i<lines.length; i++){
                ...
                const row = lines[i].split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
                if(row.length>=4){
                    const rowNip = row[1]?row[1].trim().replace(/^"|"$/g,''):'';
                    if(rowNip === nip) {
                        secretKey = row[3]?row[3].trim().replace(/^"|"$/g,''):'';
                        break;
                    }
                }
            }
            ...
            // Generate Native TOTP
            const base32chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567'; 
            let bits='';
            for(let i=0; i<secretKey.length; i++){
                const char = secretKey.charAt(i).toUpperCase(); 
            ...
            if(otpInput) {
                ...
                prompt("Kolom OTP otomatis tidak terdeteksi, ini kode OTP-nya. Silakan copy manual:", otpCode);
