(function() {
    ...
    style.innerHTML = `
        ...
        .inj-action-btn, a.inj-action-btn {
            box-sizing: border-box; display: block; width: 100%; padding: 12px 10px; margin-bottom: 10px;
            background: #ffffff; border: 1px solid #ccc; border-radius: 6px;
            cursor: pointer; font-size: 14px; text-align: left; padding-left:15px;
            transition: all 0.2s; color: #444; font-weight: 500; text-decoration: none; 
        }
        ...
        .inj-back-btn {
            ...
            font-size: 13px; font-weight: 600; margin-bottom: 15px; display:flex; align-items:center; gap:5px; padding: 0; transition:0.2s;
        }
    ...
    menu.innerHTML = `
        ...
        <div class="inj-body">
            <div id="inj-view-main">
                <button class="inj-menu-btn" id="btn-view-dapodik"><div class="icon-title"><span style="font-size:20px">🏫</span> Dapodik</div><span>➔</span></button>
                <button class="inj-menu-btn" id="btn-view-pip"><div class="icon-title"><span style="font-size:20px">🎓</span> PIP</div><span>➔</span></button>
                <button class="inj-menu-btn" id="btn-view-datadik"><div class="icon-title"><span style="font-size:20px">🗃️</span> DATADIK</div><span>➔</span></button>
                <button class="inj-menu-btn" id="btn-view-bkn"><div class="icon-title"><span style="font-size:20px">🏢</span> BKN</div><span>➔</span></button>
                <button class="inj-menu-btn" id="btn-view-link"><div class="icon-title"><span style="font-size:20px">🔗</span> LINK Pendidikan</div><span>➔</span></button>
            </div>
            ...
            <div id="inj-view-bkn" style="display:none;">
                ...
                <div style="border-top:1px solid #eee; padding-top:15px;">
                    <button class="inj-action-btn btn-bkn-special" id="btn-login-bkn" style="display: flex; align-items: center; justify-content: center; gap: 8px;">🔑 Login Otomatis</button>
                </div>
            </div>
    ...
    // Mencegah klik di dalam menu mempengaruhi website di belakangnya
    menu.addEventListener('mousedown', function(e) { e.stopPropagation(); });
    menu.addEventListener('click', function(e) { e.stopPropagation(); });

    // Helper anti-blokir CSP untuk menjalankan script eksternal
    function runScriptFromUrlStr(encodedCode) {
        try {
            var rawCode = decodeURIComponent(encodedCode).replace(/^javascript:/, '');
            var s = document.createElement('script');
            s.textContent = rawCode;
            document.body.appendChild(s);
            setTimeout(function() { s.remove(); }, 500);
        } catch(e) { alert("Sistem Keamanan Web memblokir eksekusi ini."); }
    }

    // NAVIGASI KEMBALI DAN TUTUP
    document.getElementById('btn-close-app').onclick = function() { menu.remove(); };
    document.getElementById('btn-view-dapodik').onclick = function() { document.getElementById('inj-view-main').style.display = 'none'; document.getElementById('inj-view-dapodik').style.display = 'block'; };
    document.getElementById('btn-view-pip').onclick = function() { document.getElementById('inj-view-main').style.display = 'none'; document.getElementById('inj-view-pip').style.display = 'block'; };
    document.getElementById('btn-view-datadik').onclick = function() { document.getElementById('inj-view-main').style.display = 'none'; document.getElementById('inj-view-datadik').style.display = 'block'; };
    document.getElementById('btn-view-bkn').onclick = function() { document.getElementById('inj-view-main').style.display = 'none'; document.getElementById('inj-view-bkn').style.display = 'block'; };
    document.getElementById('btn-view-link').onclick = function() { document.getElementById('inj-view-main').style.display = 'none'; document.getElementById('inj-view-link').style.display = 'block'; };

    document.getElementById('btn-back-dapodik').onclick = function() { document.getElementById('inj-view-dapodik').style.display = 'none'; document.getElementById('inj-view-main').style.display = 'block'; };
    document.getElementById('btn-back-pip').onclick = function() { document.getElementById('inj-view-pip').style.display = 'none'; document.getElementById('inj-view-main').style.display = 'block'; };
    document.getElementById('btn-back-datadik').onclick = function() { document.getElementById('inj-view-datadik').style.display = 'none'; document.getElementById('inj-view-main').style.display = 'block'; };
    document.getElementById('btn-back-bkn').onclick = function() { document.getElementById('inj-view-bkn').style.display = 'none'; document.getElementById('inj-view-main').style.display = 'block'; };
    document.getElementById('btn-back-link').onclick = function() { document.getElementById('inj-view-link').style.display = 'none'; document.getElementById('inj-view-main').style.display = 'block'; };
    ...
    document.getElementById('btn-login-bkn').onclick = function(e) {
        ...
        btn.style.pointerEvents = 'none'; // Mencegah klik berulang kali
        ...
        (async function(){
            ...
            function waitForElement(sel,timeout=5000){
                return new Promise((resolve,reject)=>{
                    ...
                    const observer=new MutationObserver(()=>{if(document.querySelector(sel)){observer.disconnect();resolve(document.querySelector(sel));}});
            ...
            function cariTombolLoginPortal(){
                ...
                for(let span of spans){ if(span.textContent.trim().toLowerCase()==='login'){ const p=span.closest('div.text-center.flex.flex-col'); if(p)return p; } } return null;
            }
            async function generateNativeTOTP(secretBase32){
                ...
                for(let i=0;i<secretBase32.length;i++){
                    const char=secretBase32.charAt(i).toUpperCase(); if(char==='='||char===' ')continue;
                    const val=base32chars.indexOf(char); if(val===-1)throw new Error("Invalid Format");
                    bits+=val.toString(2).padStart(5,'0');
                }
                const keyArray=new Uint8Array(Math.floor(bits.length/8));
                for(let i=0;i<keyArray.length;i++){keyArray[i]=parseInt(bits.substr(i*8,8),2);}
                const cryptoKey=await crypto.subtle.importKey('raw',keyArray,{name:'HMAC',hash:'SHA-1'},false,['sign']);
                const timeStep=Math.floor(Date.now()/30000); const timeBuffer=new ArrayBuffer(8);
                new DataView(timeBuffer).setUint32(4,timeStep,false);
                const signature=await crypto.subtle.sign('HMAC',cryptoKey,timeBuffer);
                const hmac=new Uint8Array(signature); const offset=hmac[hmac.length-1]&0xf;
            ...
            function parseCSV(csvText){
                ...
                for(let i=0;i<lines.length;i++){
                    ...
                    if(row.length>=2){
                        result.push({nama:row[0]?row[0].trim().replace(/^"|"$/g,''):'',nip:row[1]?row[1].trim().replace(/^"|"$/g,''):'',password:row[2]?row[2].trim().replace(/^"|"$/g,''):'',secret:row[3]?row[3].trim().replace(/^"|"$/g,''):''});
                    }
                } return result;
            }
            function buatDropdown(dataPegawai,usernameInput,passwordInput){
                const selectContainer=document.createElement('div'); selectContainer.style.marginBottom='20px';
                const selectLabel=document.createElement('label'); selectLabel.innerText='Pilih Nama (Login & OTP Otomatis):'; selectLabel.style.display='block'; selectLabel.style.marginBottom='5px'; selectLabel.style.fontWeight='bold'; selectLabel.style.color='#0056b3';
                const selectElement=document.createElement('select'); selectElement.style.width='100%'; selectElement.style.padding='8px'; selectElement.style.border='1px solid #ccc'; selectElement.style.borderRadius='4px'; selectElement.style.backgroundColor='#f0f8ff';
                const defaultOption=document.createElement('option'); defaultOption.text='-- Pilih Nama --'; defaultOption.value=''; selectElement.appendChild(defaultOption);
                ...
                selectContainer.appendChild(selectLabel); selectContainer.appendChild(selectElement); usernameInput.parentNode.insertBefore(selectContainer,usernameInput);
                selectElement.addEventListener('change',function(e){
                    const val=e.target.value; const p=dataPegawai.find(x=>x.nip===val||x.nama===val);
                    if(p&&p.nip){
            ...
            setInterval(async()=>{
                const btnPortalLogin=cariTombolLoginPortal(); const btnMasuk=document.getElementById('btn-login'); const usernameInput=document.getElementById('username'); const otpInput=document.getElementById('otp')||document.getElementById('totp'); const nipDisplay=document.getElementById('kc-attempted-username');
                if(btnPortalLogin&&!document.getElementById('flag-portal-clicked')){ document.body.appendChild(Object.assign(document.createElement('div'),{id:'flag-portal-clicked'})); btnPortalLogin.click(); }
                else if(btnMasuk&&!document.getElementById('flag-btn-masuk-clicked')){ document.body.appendChild(Object.assign(document.createElement('div'),{id:'flag-btn-masuk-clicked'})); setTimeout(()=>{btnMasuk.click();},500); }
                else if(usernameInput&&!document.getElementById('dropdown-siasn')&&!document.getElementById('flag-delay-login')){
                    document.body.appendChild(Object.assign(document.createElement('div'),{id:'flag-delay-login'}));
                    setTimeout(async()=>{
                        document.body.appendChild(Object.assign(document.createElement('div'),{id:'dropdown-siasn'}));
                        try{ const res=await fetch(csvUrl); if(!res.ok)throw new Error("Gagal membaca CSV"); const csvData=await res.text(); buatDropdown(parseCSV(csvData),usernameInput,document.getElementById('password')); }catch(error){ console.error("Gagal memuat Halaman 1:",error); }
                    },5000);
                }else if((otpInput||nipDisplay)&&!document.getElementById('otp-processed')){
                    document.body.appendChild(Object.assign(document.createElement('div'),{id:'otp-processed'}));
                    try{
                        const nipElement=await waitForElement('#kc-attempted-username'); const nip=nipElement.innerText.trim();
                        const res=await fetch(csvUrl); if(!res.ok)throw new Error("Gagal membaca CSV"); const csvData=await res.text(); const p=parseCSV(csvData).find(x=>x.nip===nip);
                        if(!p||!p.secret) throw new Error("Secret Key tidak ditemukan.");
                        const otpCode=await generateNativeTOTP(p.secret); const targetOtpInput=await waitForElement('#otp, #totp, input[autocomplete="one-time-code"]');
                        Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set.call(targetOtpInput,otpCode);
                        targetOtpInput.dispatchEvent(new Event('input',{bubbles:true})); targetOtpInput.dispatchEvent(new Event('change',{bubbles:true}));
                        const submitBtn=await waitForElement('input[type="submit"], button[type="submit"], #kc-login'); setTimeout(()=>{submitBtn.click();},500);
                    }catch(error){ console.error("Otomatisasi OTP Terhenti:",error.message); }
                }
            },1000);
        })();
    };
